...
export class VoteGuardianAPI implements DeployedVoteGuardianAPI {
  ...
  private constructor(
    ...
    this.state$ = combineLatest(
      [
        ...
        from(providers.privateStateProvider.get('voteGuardianPrivateState3') as Promise<VoteGuardianPrivateState>),
      ],
    ...
    try {
      ...
      */
      const DeployedVoteGuardianContract = await deployContract(providers, {
        privateStateId: 'voteGuardianPrivateState3',
  ...
  static async join(
    ...
    const deployedVoteGuardianContract = await findDeployedContract(providers, {
      ...
      initialPrivateState:
        // (await providers.privateStateProvider.get('voteGuardianPrivateState3')) ??
